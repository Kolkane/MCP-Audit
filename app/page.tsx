    ...
    <main className="bg-background text-night">
      ...
      <AnimatePresence mode="wait">
        {analysisState === "idle" && (
          ...
          >
            <IdleSections calendlyUrl={calendlyUrl} />
          </motion.div>
        )}
      </AnimatePresence>
      <TestimonialsSection />
...
function IdleSections({ calendlyUrl }: { calendlyUrl: string }) {
  return (
    <>
      <ProblemSection />
      <ReportSection />
      <ProcessSection />
      <PersonasSection />
      <PricingSection calendlyUrl={calendlyUrl} />
    </>
  );
}
        ...
        <FadeIn>
          <div className="flex flex-col gap-12 md:flex-row md:items-center">
            <div className="flex-1 space-y-4">
              ...
              <a href="#hero" className="inline-flex items-center justify-center rounded-2xl bg-accent px-6 py-3 text-sm font-semibold text-white">
            ...
            <div className="flex-1">
              <div className="relative rotate-1">
                <div className="absolute inset-0 -rotate-3 scale-95 rounded-3xl bg-indigo-100 opacity-70" />
                          ...
                          <div className="h-2 w-[160px] rounded-full bg-[#F1F5F9]">
                            <div
                              ...
                              style={{
                                ...
                                background: item.score >= 14 ? "#22C55E" : item.score >= 9 ? "#F97316" : "#EF4444"
                              }}
                            />
                          </div>
...
function PricingSection({ calendlyUrl }: { calendlyUrl: string }) {
  return (
        ...
        <FadeIn>
          <div className="grid gap-6 md:grid-cols-3">
            {testimonials.map((item, index) => (
              <div key={item.label} className={`rounded-2xl border border-border bg-white p-8 shadow-[0_4px_24px_rgba(0,0,0,0.06)] ${index === 0 ? "md:-rotate-1" : index === 2 ? "md:rotate-1" : "md:rotate-0"}`}>
                ...
                <div className="flex items-center justify-between">
                  <span className="rounded-full border border-green-200 bg-green-50 px-3 py-1 text-sm font-bold text-green-700">{item.delta} ↑</span>
      ...
      <div className="mx-auto max-w-4xl rounded-3xl bg-gradient-to-br from-[#4F46E5] to-accent p-12 text-center text-white">
        <FadeIn>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            ...
            <a href="#pricing" className="rounded-xl border-2 border-white/60 px-6 py-4 text-base font-semibold text-white transition hover:border-white hover:bg-white/10">
              Voir les offres
            </a>
          </div>
    ...
    <footer className="bg-night py-10 text-sm text-[#94A3B8]">
      <div className="mx-auto flex max-w-6xl flex-col gap-4 px-6 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap gap-6 text-sm">
          <Link className="text-white/80" href="/mentions-legales">
            Mentions légales
          </Link>
          <a href="mailto:contact@agentable.fr" className="text-white/80">
            Contact
          </a>
          <a className="text-white/80" target="_blank" rel="noreferrer" href="https://calendly.com/agentable/demo">
